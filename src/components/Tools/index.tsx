...
export default () => {
    ...
    return (
        <>
            <div
                className="z-[999] overflow-hidden fixed top-[70%] right-[3%] flex flex-col w-12 bg-white dark:bg-black-b border dark:border-[#4e5969] rounded-md divide-y dark:divide-[#4e5969]  ">
                {isDark ? <Image src={sun.src} alt="太阳" width={46} height={46} className={itemSty}
                                 onClick={() => setIsDark(false)}/> :
                    <Image src={moon.src} alt="月亮" width={46} height={46} className={itemSty}
                           onClick={() => setIsDark(true)}/>}

                <Image src={search.src} alt="搜索" width={46} height={46} className={itemSty}
                       onClick={onSwiperOpenChange}/>
                <Image src={rss.src} alt="订阅" width={46} height={46} className={itemSty} onClick={onRssOpenChange}/>
                <Image src={returnTop.src} alt="返回顶部" width={46} height={46} className={itemSty}
                       onClick={onReturnTop}/>
            </div>
