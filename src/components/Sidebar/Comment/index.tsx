    ...
    return (
        <div className="flex flex-col tw_container bg-white dark:bg-black-b p-4 mb-5 tw_title">
            ...
            <div className="mt-2.5">
                {list?.map((item) => (
                    <Link href={`/article/${item.articleId}`} target="_blank"
                        ...
                        {item.avatar ? <img src={item.avatar}
                                            ...
                                            alt="avatar"/> : <RandomAvatar
                            className="w-11 h-11 rounded-full mr-2.5 transition-transform hover:scale-110"/>}

                        <div className="flex flex-col justify-center">
                            <div
                                className="w-48 text-sm text-gray-600 dark:text-[#8c9ab1] group-hover:text-primary overflow-hidden line-clamp-2">{item.content}</div>
                            <div
                                className="pt-2.5 text-xs text-gray-400">{dayjs(+item.createTime!).format('YYYY-MM-DD HH:mm')}</div>
                        </div>
                    </Link>
                ))}
            </div>
