    ...
    return (
        <>
            <div
                ...
                {data?.map((item, index) => (
                    <div key={index}
                         className={`absolute top-0 left-0 w-full h-full ${index === current ? 'opacity-100' : 'opacity-0'} transition-opacity`}>
                        <img key={index} src={item.image} alt={item.title} className="w-full h-full object-cover"
                             width={1694} height={700}/>
                        ...
                        <div className="flex flex-col absolute bottom-7 left-5 z-10 gap-2 animation_bottom">
                            ...
                            <Button color="primary" variant="shadow" endContent={<BiChevronRight className="text-2xl"/>}
                                    className="!w-28 mt-2 hover:translate-x-2 transition-transform"
