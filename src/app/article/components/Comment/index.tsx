    ...
    return (
        <div className="CommentComponent">
            <div className="mt-[70px]">
                ...
                <form className="flex flex-wrap justify-between mt-4 space-y-2 text-xs xs:text-sm"
            ...
            />
                        ...
                        <div className="absolute bottom-5 right-5">
                            <Popover placement="bottom" isOpen={isEmojiOpen} onOpenChange={setIsEmojiOpen}>
                                <PopoverTrigger>
                                    <button type="button"
                                            className="py-1 px-2 text-2xl rounded-md hover:bg-gray-50 border border-transparent hover:border-gray-200">
                                        😀
                                    </button>
                                </PopoverTrigger>
                    ...
                    <div className="flex flex-col w-[32%]">
                        <input type="text" className="tw_form w-full h-9 pl-4"
                               placeholder="你的名称" {...register('name', {required: '请输入名称'})} />
                        <span className="text-red-400 text-sm pl-3 mt-1">{errors.name?.message}</span>
                    </div>

                    <div className="flex flex-col w-[32%]">
                        <input type="text" className="tw_form w-full h-9 pl-4"
                               placeholder="你的邮箱（选填）" {...register('email', {
                            pattern: {
                    ...
                    <div className="flex flex-col w-[32%]">
                        <input type="text" className="tw_form w-full h-9 pl-4"
                               placeholder="头像（选填）" {...register('avatar', {
                            pattern: {
                                value: /^https?:\/\//,
                                message: '请输入正确的头像链接'
                            }
                    ...
                    <div className="w-full flex flex-col">
                        <input type="text" className="tw_form w-full h-9 pl-4"
                               placeholder="你的站点（选填）" {...register('url', {
                            pattern: {
                                value: /^https?:\/\//,
                                message: '请输入正确的网站链接'
                            }
                    ...
                    {loading ? (
                        ...
                        <button className="w-full h-10 !mt-4 text-white rounded-md bg-primary text-center"
                                type="submit">
                            发表评论
                        </button>
                    )}
